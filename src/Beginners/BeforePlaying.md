...
::: info 
- <font color=orange size=4><strong>在练习模式选人界面按 C键 打开菜单</strong></font>

- 若要修改机签名，请打开你的 `th123` 游戏文件夹，找到 `profile` 文件夹，进入后将 `profile1p.pf` 修改为你想要的文件名即可，如修改为 `入则新人.pf`，修改后重启游戏。
...
![右上角就是选择好了的，正在使用的机签名，如图中是 `Tstar00（2）`](https://bu.dusays.com/2024/03/27/6603aa1341843.png =600x)
...
按下 `Win+R`，键入或粘贴命令 `cmd /k ping baidu.com -t` 后回车，稍微等待一会，
延迟有上下 15ms 左右的波动都属于正常情况。
...
::: note Swarm 通过中转服务器，中转数据包的方式来帮助联机，只需要建主方打开 Swarm 软件即可，对连接方无要求
:::
