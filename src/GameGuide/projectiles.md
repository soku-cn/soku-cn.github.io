...
![图片加载中...](https://bu.dusays.com/2023/11/29/6566e63132a60.gif "【同帧相杀可以“以一当多”】  同一帧时，灵梦C弹一次相杀次数就和4个刀子弹幕抵消，剩下的两次相杀次数即为2HIT")
...
![图片加载中...](https://bu.dusays.com/2023/11/30/65682a91ab580.gif "236 火球速度较快且相杀时不减速，会互相穿透而不是互相抵消")

![图片加载中...](https://bu.dusays.com/2023/11/30/65682c60c60a1.gif "高达 5C 由于相杀时减速而互相抵消，不能穿透")

![图片加载中...](https://bu.dusays.com/2023/11/30/6568299e83581.gif "高达5C弹幕相杀时减速，假如不减速的话图中帕秋莉的H6C是来不及消除掉高达5C而被命中的")

![图片加载中...](https://bu.dusays.com/2023/11/30/65682db03b44e.gif "帕秋莉 H6C 和 236B 版火球不能穿透阳伞，236C 版高速火球可以在10F时间内穿透阳伞命中对手")
...
![图片加载中...](https://bu.dusays.com/2023/11/30/65684995c9571.gif " ")
...
::: info
身代属性的意思是，弹幕含有受击判定。对手打在拥有此属性的弹幕上，就像打在一个角色身上一般，会有攻击命中特效，算作接触成功，可以触发接触取消，且对手要承受 Hitstop，攻击判定也会因此失效

部分身代属性弹幕还含有血量，当血量减少到 0 以下时身代判定与攻击判定同时消失（比如有3000血量，那要受到3001伤害才会消失）。身代所受到的伤害不会受到修正与特殊补正的影响，假设面对 `4 HIT，伤害 600，Rate 90% ，Limit 50%`的攻击，身代会收到 `4*600=2400` 的伤害，而玩家只会受到 `600+600*90%*(9400/10000*30%+70%)=1130`的伤害

但是，无论多么脆弱的身代判定至少也能抵挡 1 HIT 攻击，因此面对单发高伤的攻击时很实用，如魔理沙的五卡彗星，八云紫的五卡火车等等

![图片加载中... =400x](https://bu.dusays.com/2024/02/16/65cf36165e792.gif "天子 22 石柱身代，妖梦 6A 这种同时命中石柱和天子则可有效攻击，而 DA 和 22半灵 攻击被石柱吸收抵挡")

![图片加载中... =400x](https://bu.dusays.com/2024/02/16/65cf355ab8ceb.gif "人偶挡火车")


**具有身代属性的招数诸如：**
- 妖梦的 6C、原22 发射状态的半灵；幽冥状态下带可被打康属性的半灵（应该算是bug）
- 爱丽丝的 部分激活状态的人偶（发射弹幕时的C人偶，236人偶，原22激活人偶，623改大江户人偶等等）
- 天子的 原22 石柱（落地状态）
- 早苗的 原214 青蛙盾
- ⑨的 2C 冰球；原214 推好释放的雪球等等
- 帕秋莉的 22改大水泡；三卡泡泡盾；四卡大绿石等
- 萃香的 原214 小瓜
- 等等
...
### 被打不消失（発生保障）
::: info
**通常情况下，当一个角色受击时，属于他的弹幕会立即消失并转化为天气玉**

**然而某些招数弹幕，则拥有 “被打不消失” 的特性，意味着即使使用者受击，在场的那个弹幕也不会失效消失**
...
![图片加载中... =400x](https://bu.dusays.com/2024/02/16/65cf3e59e45a8.gif "天子被妖梦打了，22 生成的石柱也不会消失，依然可以打到妖梦")


::: tip

**特殊地，还有部分招数弹幕是 “削弱版的被打不消失”**
**意思是受击但不击飞的情况下，弹幕不会消失；但若被击飞，弹幕还是会消失转化为天气玉**
...
- 等等
- 
![图片加载中... =400x](https://bu.dusays.com/2024/02/16/65cf3cb82df3b.gif "灵梦 原214 警醒阵，被摸一下不会消失，被击飞就消失了")

![图片加载中... =400x](https://bu.dusays.com/2024/02/16/65cf3cd4725cc.gif "咲夜 214改 单发弹刀，被摸一下不会消失，被击飞就消失了")
...
::: info
**全游戏的反射技只有这两个：**
1. 衣玖的 623改 羽衣若风（可空中使用）

2. 妖梦的 原236 反射盾，反射原弹幕的同时也会生成妖梦自己的弹幕
 `冷知识：妖梦原 236 反射产生的附加弹幕，看起来和妖梦的 B 弹幕一样，实际性质完全不一样。攻击判定不一样；伤害数据不一样；命中音效和效果也不一样，B弹幕是会强制受身，而反射弹幕会击飞，且反射弹幕相杀等级比 C 还低，会被 C 级弹幕瞬间消除而不影响那个 C 级弹幕，所以是唯一的 D 级弹幕`
 `1.03 以前的版本中，妖梦满级反射盾会附带弹幕攻击判定，但 1.10 版本削掉了这个攻击判定`

**弹幕能否反射都是比较独立的，单独设置的属性（类似当身）**
...
对于反射技，假设妖梦反射掉了图书的火球，只能将反射回去的火球的攻击（相杀）判定化为己用，其使用者依然是图书。

若此时妖梦受击，反射盾等弹幕会消失，但是火球不会消失依然可以命中图书；若此时图书提前受击，则火球会消失。因此，若妖梦反射掉了蕾咪的 C 弹，这个 C 弹命中蕾咪是，只能命中第 1 HIT，然后就会消失而不能打满 10 HIT；若是蕾咪防御了这个 C 弹，则会吃满 10 HIT。
...
![图片加载中... =400x](https://bu.dusays.com/2024/02/16/65cf5d2eba843.gif "红美铃的 236改 水形太极拳")

**吸收实际就是弹幕相杀，吸收盾是 B 级无限相杀，相杀不过或无相杀的则不能吸收**
:::
