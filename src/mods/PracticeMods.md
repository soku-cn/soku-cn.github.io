...
## practice-position-lock lua mod
::: info 下载安装

[点击下载 practice-position-lock.zip](https://gitee.com/sanhuizhuan/SokuMods/releases/download/v1/practice-position-lock.zip)

将 practice-position-lock.zip 放入 `th123\modules\shady-loader\` 文件夹，在 F2 菜单内开关（类似美化包）
...
practice-position-lock lua mod 只在练习模式生效

- 按大键盘 6 键，将 2P 木桩锁定在空中或地上
- 按大键盘 1 键，恢复木桩位置，不开 Labtool 也可以恢复位置，但是更推荐搭配使用，这个mod的恢复功能不会像 Labtool 那样发生卡死的情况
- 锁定和复位两个热键可以在 practice-position-lock.ini 文件内修改，键位请看图片。恢复键建议和 Labtool 的键位保持一致不要改，使用大键盘 1 键
