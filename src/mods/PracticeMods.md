...
- F2 卡槽补满
- F3 卡槽清空且重置卡组
- **注意：开启 [无限套卡组 mod](/mods/AdvancedMods/InfiniteDeck.html) 后，木桩的卡组是你 1P 自己机签的卡组**

::: warning 按 F1、F2、F3 这些键都没有效果可能是因为 Fn Lock 的问题

[**百度搜索“Fn锁定”**](https://www.baidu.com/s?ie=UTF-8&wd=Fn%E9%94%81%E5%AE%9A)  一般按 `Fn + ESC` 或者 `Fn + 空格` 可以解除锁定，或者按 `Fn+F2`，`Fn+F3` 来使用热键
:::
...
- 按 `PAGE UP` 键切换下一个动作
- 也可以在`录制的动作`这一页中上下选择然后按 Z键来切换到那个动作
...
- ❌ **录制动作超过 100帧 的话，不要使用精简功能，否则会有无法精简且动作重复翻倍的 bug**
...
| 6  |  开关 HUD 显示，单机和观战可用。关闭 HUD 后有无法使用木桩动作的 Bug<br>Rep和观战时不会自动恢复HUD，其他模式会 |
| 0  |  是否允许木桩回血 |
| 左 Ctrl + F  |  解锁/锁定 1P |
| 左 Ctrl + G  |  解锁/锁定 2P |
...
- [ReplayInputView++ 更新 By 鱼的千千](https://www.bilibili.com/read/cv43285231)
...
- [[东方非想天则]打桩模式录制功能的使用案例-哔哩哔哩](https://b23.tv/1AxN4uh)

- **在暂停状态下，按住某个键，再按 F12 即可逐帧输入按键**
❌ 注意有可能遇到 **三键冲突** 的情况，会导致按不下去 F12，此时可以修改 `ReplayInputView++.ini` 内的热键，把F12改成别的不冲突的键，或用键盘重映射软件临时把F12改成别的，或者改机签的方向键键位，或者用两个键盘（笔记本自带的键盘和外接键盘）
